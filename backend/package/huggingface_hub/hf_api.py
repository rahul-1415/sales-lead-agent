...
def _parse_hf_copy_handle(hf_handle: str) -> _BucketCopyHandle | _RepoCopyHandle:
    ...
    if not hf_handle.startswith("hf://"):
        raise ValueError(
            f"Invalid HF handle: '{hf_handle}'. Expected a path starting with 'hf://'."
        )

    path = hf_handle.removeprefix("hf://")
    if path.startswith("buckets/"):
        bucket_id, bucket_path = _split_bucket_id_and_prefix(
...
class HfApi:
    ...
    def list_models(
        ...
        *,
        ...
        inference_provider: (
            Literal["all"] | PROVIDER_T | list[PROVIDER_T] | None
        ) = None,
    ...
    def _create_uv_command_env_and_secrets(
        ...
        *,
        ...
        if len(local_files_to_include) == 0:
                ...
                if remote_file_path.name in remote_to_local_file_names:
                    for i in itertools.count():
                        ...
                        if remote_file_name not in remote_to_local_file_names:
                            remote_to_local_file_names[remote_file_name] = (
                                local_file_to_include
                            )
                            break
                else:
                    remote_to_local_file_names[remote_file_path.name] = (
                        local_file_to_include
                    )
            ...
            script_args = [
                (
                    local_to_remote_file_names[arg]
            ...
            env["LOCAL_FILES_ENCODED"] = "\n".join(
                remote_file_name
                + " "
                + base64.b64encode(Path(local_file_to_include).read_bytes()).decode()
                for remote_file_name, local_file_to_include in remote_to_local_file_names.items()
            )
            # Shell-quote each arg to prevent metacharacters (e.g. '>') from being interpreted by bash
            quoted_parts = [
                "'" + arg.replace("'", r"'\''") + "'"
                for arg in [*uv_args, script, *script_args]
            ]
            command = [
                "bash",
                "-c",
    ...
    def copy_files(
        ...
        if destination_path == "":
            ...
            if dest_path_info:
                ...
                destination_is_directory = (
                    destination_exists_as_directory or destination.endswith("/")
                )
        ...
        pending_downloads: list[tuple[str, str]] = (
            []
        )  # (file_path, target_path) for non-xet files to download
        ...
        if isinstance(source_handle, _BucketCopyHandle):
            ...
            source_path_info = list(
                self.get_bucket_paths_info(
                    source_handle.bucket_id, [source_path], token=token
                )
            )
        ...
        # === Source is a repo: copy-by-hash if xet-backed, download otherwise ===
            ...
            if len(source_repo_path_info) == 1 and isinstance(
                ...
                if source_repo_path_info[0].path.rsplit("/", 1)[-1] == ".gitattributes":
                    return
                target_path = _resolve_target_path(
                    source_repo_path_info[0].path, None, is_single_file=True
                )
        ...
        *,
        ...
        if len(add_operations_to_upload) > 0:
            ...
            def token_refresher() -> tuple[str, int]:
                ...
                return (
                    new_xet_connection.access_token,
                    new_xet_connection.expiration_unix_epoch,
                )
            ...
            try:
                ...
                for upload_info, op in zip(
                    xet_upload_infos,
                    [op for op in add_path_operations if op.xet_hash is None],
                ):
                ...
                for upload_info, op in zip(
                    xet_upload_infos,
                    [op for op in add_bytes_operations if op.xet_hash is None],
                ):
                    op.xet_hash = upload_info.hash
                    op.size = upload_info.filesize
    ...
    def download_bucket_files(
        ...
        *,
        ...
        connection_info = refresh_xet_connection_info(
            file_data=metadata.xet_file_data, headers=headers
        )
