...
def load_marketplace_skills() -> list[MarketplaceSkill]:
    ...
    for plugin in plugins:
        ...
        if not isinstance(name, str) or not isinstance(source, str):
            continue
        skills.append(
            MarketplaceSkill(name=name, repo_path=_normalize_repo_path(source))
        )
    return skills
...
def install_marketplace_skill(
    skill: MarketplaceSkill, destination_root: Path, force: bool = False
) -> Path:
...
def check_for_updates(
    ...
    marketplace_skills = {
        skill.name.lower(): skill for skill in load_marketplace_skills()
    }
    updates = [
        _evaluate_update(skill_dir, marketplace_skills)
        for skill_dir in _iter_unique_skill_dirs(roots)
    ]
...
def read_installed_skill_manifest(
    skill_dir: Path,
) -> tuple[InstalledSkillManifest | None, str | None]:
...
def write_installed_skill_manifest(
    skill_dir: Path, manifest: InstalledSkillManifest
) -> None:
    payload = {
...
def _load_marketplace_payload() -> dict[str, Any]:
    ...
    try:
        ...
        raise CLIError(
            f"Failed to decode GitHub API response for 'contents/{MARKETPLACE_PATH}': {exc}"
        ) from exc
    if not isinstance(payload, dict):
...
def _extract_remote_github_path(
    revision: str, source_path: str, install_dir: Path
) -> None:
...
def _extract_tar_subpath(tar_bytes: bytes, source_path: str, install_dir: Path) -> None:
    ...
    with tarfile.open(fileobj=io.BytesIO(tar_bytes), mode="r:*") as archive:
        ...
        for member in members:
            relative_parts = _member_relative_parts(
                member_name=member.name, source_parts=source_parts
            )
...
def _member_relative_parts(
    member_name: str, source_parts: tuple[str, ...]
) -> tuple[str, ...] | None:
    ...
    if (
        len(path_parts) > len(source_parts)
        and tuple(path_parts[1 : 1 + len(source_parts)]) == source_parts
    ):
...
def _evaluate_update(
    skill_dir: Path, marketplace_skills: dict[str, MarketplaceSkill]
) -> SkillUpdateInfo:
    ...
    if manifest is None:
        return replace(
            base, status="invalid_metadata" if error else "unmanaged", detail=error
        )
    ...
    status: SkillUpdateStatus = (
        "up_to_date" if available_revision == current_revision else "update_available"
    )
    return replace(
        base,
        status=status,
...
def _filter_updates(
    updates: list[SkillUpdateInfo], selector: str | None
) -> list[SkillUpdateInfo]:
    if selector is None:
        return updates
    selector_lower = selector.strip().lower()
...
def _resolve_available_revision(skill: MarketplaceSkill) -> str:
    ...
    try:
        ...
        raise CLIError(
            f"Failed to decode GitHub API response for 'commits': {exc}"
        ) from exc
    if not isinstance(payload, list) or not payload:
        raise CLIError(
            f"Unable to resolve the current revision for skill '{skill.name}'."
        )
...
def _fetch_from_skills_repo(endpoint: str, params: dict[str, Any] | None = None) -> Any:
    ...
    try:
        ...
        raise CLIError(
            f"Failed to fetch '{endpoint}' from {DEFAULT_SKILLS_REPO_ID}: {exc}"
        ) from exc
    return response
