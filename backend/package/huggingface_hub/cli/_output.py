...
class Output:
    ...
    def set_mode(self, mode: OutputFormatWithAuto = OutputFormatWithAuto.auto) -> None:
        ...
        if mode == OutputFormatWithAuto.auto:
            mode = (
                OutputFormatWithAuto.agent if is_agent() else OutputFormatWithAuto.human
            )
        self.mode = mode
    ...
    def table(
        ...
        *,
        ...
        match self.mode:
            case (
                OutputFormatWithAuto.human
            ):  # padded table, truncated cells, SCREAMING_SNAKE headers
                ...
                screaming_alignments = {
                    _to_header(k): v for k, v in (alignments or {}).items()
                }
                print(
                    tabulate(
                        formatted_rows,
            ...
            case (
                OutputFormatWithAuto.quiet
            ):  # id_key column (or first column), one per line
    ...
    def confirm(
        self, message: str, *, default: bool = False, yes: bool = False
    ) -> None:
        """
