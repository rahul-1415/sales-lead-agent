...
class HFCliTyperGroup(TyperGroup):
    ...
    def _rewrite_quiet_shorthand(cmd: click.Command, args: list[str]) -> None:
        ...
        has_quiet_option = any(
            ...
            and ("--quiet" in param.opts or "-q" in param.opts)
            for param in cmd.params
        )
        if has_format_option and not has_quiet_option:
            if any(arg == "--format" or arg.startswith("--format=") for arg in args):
                raise click.UsageError(
                    "'--quiet' and '--format' are mutually exclusive."
                )
...
def print_list_output(
    ...
    if headers is None:
        ...
        headers = [
            ...
            if any(_format_cell(item.get(col)) for item in items)
        ]
...
def _prompt_autoupdate(
    ...
    click.echo(
        ANSI.yellow("  Do you want to update now? [Y/n] ")
        + ANSI.gray(f"({display_cmd})")
        + " ",
        file=sys.stderr,
        nl=False,
    )
    ...
    )  # Note: if user press 'Enter', raw_answer is `\n`
    if answer in ("", "y", "yes"):
        click.echo("", file=sys.stderr)
        ...
        if returncode == 0:
            ...
            click.echo(
                ANSI.green(
                    f"  ✓ Successfully updated {library} to {latest_version}. Please re-run your command."
                ),
                file=sys.stderr,
            )
            raise SystemExit(0)
            ...
            click.echo(
                ANSI.red(
                    f"  ✗ Update failed (exit code {returncode}). Please update manually."
                ),
                file=sys.stderr,
            )
    else:
