...
def _list_buckets(
    ...
    api = get_hf_api(token=token)
    results = [
        api_object_to_dict(bucket) for bucket in api.list_buckets(namespace=namespace)
    ]

    if not results:
        if not quiet and format != OutputFormat.json:
            resolved_namespace = (
                namespace if namespace is not None else api.whoami()["name"]
            )
...
def cp(
    src: Annotated[
        str,
        typer.Argument(
            help="Source: local file, any hf:// handle (model, dataset, bucket), or - for stdin"
        ),
    ],
    dst: Annotated[
        str | None,
        typer.Argument(
            help="Destination: local path, bucket hf://... handle, or - for stdout"
        ),
    ] = None,
    quiet: QuietOpt = False,
    token: TokenOpt = None,
    ...
    if not src_is_bucket and not src_is_stdin and os.path.isdir(src):
        raise typer.BadParameter(
            "Source must be a file, not a directory. Use `hf buckets sync` for directories."
        )
    ...
    if src_is_bucket:
        ...
        if prefix == "" or prefix.endswith("/"):
            raise typer.BadParameter(
                "Source path must include a file name, not just a bucket or directory path."
            )
