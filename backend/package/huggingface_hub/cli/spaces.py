...
def spaces_search(
    ...
    for r in itertools.islice(results, limit):
        item: dict = {
            ...
            "score": (
                round(r.semantic_relevancy_score, 2)
...
def dev_mode(
    ...
    print(
        f"Dev mode is currently building, track the progress here: https://huggingface.co/spaces/{info.id}"
    )
...
def spaces_hot_reload(
    ...
    if local_file:
        ...
        if editor_res == "no-tty":
            raise CLIError(
                "Cannot open an editor (no TTY). Use -f flag to hot-reload from local path"
            )
...
def volumes_set(
    ...
    out.result(
        "Volumes set", space_id=space_id, volumes=[v.to_hf_handle() for v in volumes]
    )
