...
def _print_output(
    rows: list[list[str | int]], headers: list[str], aliases: list[str], fmt: str | None
) -> None:
...
def jobs_ps(
    ...
    format: Annotated[
        str | None,
        typer.Option(
            help="Output format: 'table' (default), 'json', or a Go template (e.g. '{{.id}}')"
        ),
    ] = None,
    ...
    if not filtered_jobs:
        if not quiet and format != "json":
            filters_msg = (
                f" matching filters: {', '.join([f'{k}{o}{v}' for k, o, v in filters])}"
                if filters
                else ""
            )
...
def jobs_hardware() -> None:
    ...
    for hw in hardware_list:
        ...
        cost_hour = (
            f"${hw.unit_cost_usd * 60:.2f}" if hw.unit_cost_usd is not None else "N/A"
        )
        rows.append(
            [
                hw.name,
                hw.pretty_name or "N/A",
                hw.cpu,
                hw.ram,
                accelerator_info,
                cost_min,
...
def scheduled_inspect(
    ...
    print(
        json.dumps(
            [asdict(scheduled_job) for scheduled_job in scheduled_jobs],
            indent=4,
            default=str,
        )
    )
