...
class SoftReadWriteLock(metaclass=_SoftRWMeta):
    ...
    def _acquire(
        ...
        *,
        ...
        if not effective_blocking:
            ...
            acquired = self._locks.transaction.acquire(
                blocking=True, timeout=effective_timeout
            )
        if not acquired:
    ...
    def _refresh_marker(self) -> bool:
        ...
        try:
            ...
            FileNotFoundError
        ):  # pragma: no cover - race between successful read and touch
            return False
        return True

    def _reset_after_fork_in_child(
        self,
    ) -> None:  # pragma: no cover - fork child not tracked
...
def _read_marker(
    ...
    try:
        try:
            ...
            OSError
        ):  # pragma: no cover - fstat/read after a successful open is hard to provoke
            return None
    finally:
        os.close(fd)
...
def _reset_all_after_fork() -> (
    None
):  # pragma: no cover - fork child, not tracked by coverage
