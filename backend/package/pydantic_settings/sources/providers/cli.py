...
class _CliArg(BaseModel):
    ...
    def __init__(
        ...
        parser_map: defaultdict[
            str | FieldInfo, dict[int | None | str | type[BaseModel], _CliArg]
        ],
        **values: Any,
    ) -> None:
...
class CliSettingsSource(EnvSettingsSource, Generic[T]):
    ...
    def _merged_list_to_str(self, merged_list: list[str], field_name: str) -> str:
        ...
        try:
            ...
            is_num_type_str = (
                type(next(iter(list_adapter.validate_python(["1"])))) is str
            )
        except Exception:
        ...
        for field_name, field_info in self._sort_arg_fields(model):
            ...
            if arg.subcommand_dest:
                for sub_model in arg.sub_models:
                    ...
                    subcommand_arg.kwargs["description"] = _get_model_description(
                        sub_model
                    )
                    ...
                    if self.cli_use_class_docs_for_groups:
                        subcommand_arg.kwargs["help"] = _get_model_description(
                            sub_model
                        )
        ...
        if kwargs["metavar"] == "bool":
            ...
            if meta_bool_flags:
                ...
                if bool_flag is _CliDualFlag:
                    ...
                    del kwargs["metavar"]
                    kwargs["action"] = (
                        "store_false" if field_info.default else "store_true"
                    )
