...
class MultimodalInputSegmentImageURL(BaseModel):
    type: Literal[MultimodalInputSegmentType.IMAGE_URL] = (
...
class MultimodalInputSegmentImageBase64(BaseModel):
    type: Literal[MultimodalInputSegmentType.IMAGE_BASE64] = (
...
class MultimodalInputSegmentVideoURL(BaseModel):
    type: Literal[MultimodalInputSegmentType.VIDEO_URL] = (
...
class MultimodalInputSegmentVideoBase64(BaseModel):
    type: Literal[MultimodalInputSegmentType.VIDEO_BASE64] = (
...
class MultimodalInputRequest(BaseModel):
    ...
    def _process_list_input(
        ...
        if not all(
            isinstance(item, (str, PIL.Image.Image, Video)) for item in input_list
        ):
            raise ValueError(
