...
def _parse_file(content: bytes, filename: str) -> list[RawLead]:
    try:
        if filename.endswith(".csv"):
            ...
            raise HTTPException(
                status_code=415, detail="Only .csv and .json files are supported"
            )
...
async def upload_leads(file: UploadFile = File(...)):
    ...
    if settings.is_local:
        ...
        return await _upload_aws(
            content, leads, filename, batch_id, job_id, file.content_type
        )
...
async def _upload_local(
    ...
    for lead in leads:
        ...
        if key in existing_keys or key in seen_this_batch:
            duplicate_count += 1
            logger.info(
                "duplicate skipped", extra={"company": lead.company, "key": key}
            )
        else:
    ...
    results = process_batch(
        leads=unique_leads, batch_id=batch_id, job_id=job_id, stats=stats
    )
    ...
    logger.info(
        "local batch processed",
        extra={"job_id": job_id, "count": processed, "duplicates": duplicate_count},
    )
...
def get_download_url(job_id: str):
    if settings.is_local:
        raise HTTPException(
            status_code=501, detail="Download not available in local mode."
        )
    item = db.get_job(job_id)
    if not item:
        _job_not_found(job_id)
    ...
    return {
        "download_url": storage.presigned_download_url(job.s3_output_key),
        "expires_in": 3600,
    }
