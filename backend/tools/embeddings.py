    ...
    try:
        ...
        for model in _VOYAGE_MODELS:
            try:
                ...
                logger.warning("Voyage model unavailable", extra={"model": model, "error": str(e)})
        ...
        logger.warning("Voyage AI init failed — using Jaccard similarity", extra={"error": str(e)})
        return False
...
def _voyage_embed(texts: list[str], input_type: str) -> Optional[list[list[float]]]:
    ...
    try:
        ...
        logger.warning("Voyage embed call failed — falling back to Jaccard", extra={"error": str(e)})
        return None
...
def build_icp_index() -> None:
    ...
    if _init_voyage():
        ...
        if embeddings:
            _voyage_index = list(zip([ex["company"] for ex in _ICP_EXAMPLES], embeddings))
            logger.info("ICP Voyage index ready", extra={"model": _voyage_model, "count": len(_voyage_index)})
        else:
