...
_INDUSTRY_KEYWORDS: dict[IndustrySegment, list[str]] = {
    IndustrySegment.LOGISTICS: ["logistics", "freight", "shipping", "supply chain", "transport", "warehouse"],
    IndustrySegment.MANUFACTURING: ["manufacturing", "factory", "industrial", "fabrication", "assembly"],
    IndustrySegment.RETAIL: ["retail", "ecommerce", "e-commerce", "consumer goods", "wholesale"],
    IndustrySegment.HEALTHCARE: ["healthcare", "health", "medical", "pharma", "biotech", "clinical"],
    IndustrySegment.FINANCIAL_SERVICES: ["finance", "banking", "insurance", "fintech", "investment", "payments"],
    IndustrySegment.TECHNOLOGY: ["software", "saas", "technology", "tech", "cloud", "ai", "data"],
    IndustrySegment.ENERGY: ["energy", "oil", "gas", "utilities", "renewables", "power"],
}
...
def _mock_enrich(company_name: str) -> CompanyEnrichment:
    ...
    description: str = data.get("description", f"{company_name} — no description available.")
...
def _clearbit_enrich(company_name: str) -> CompanyEnrichment:
    ...
    try:
        ...
        logger.warning("Clearbit enrichment failed — falling back to mock", exc_info=True)
        return _mock_enrich(company_name)
